...
class _ConsultaCidadeState extends State<ConsultaCidade> {
  ...
  Widget build(BuildContext context) {
    ...
    listaTodos() async {
      List<Cidade> pessoas = await AcessoApi().listaCidade();
      setState(() {
        lista = pessoas;
      });
    }
    ...
    return Scaffold(
      ...
      body: Form(
        ...
        child: Column(
          children: [
            Componentes()
                .criaBotao(formController, listaTodos, "Listar Cidades"),
            Expanded(
              child: Container(
  ...
  poupupDelete(BuildContext context, Cidade cidade) {
    return showDialog(
        context: context,
        builder: (context) => AlertDialog(
              title: const Text('DESEJA DELETAR?'),
              content:
                  Text("Você realmente deseja delete a cidade ${cidade.nome}?"),
              actions: [
                TextButton(
